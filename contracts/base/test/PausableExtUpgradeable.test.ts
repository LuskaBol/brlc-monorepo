...
for (const contractName of ["PausableExtUpgradeable", "PausableEnumerableExtUpgradeable"]) {
  describe(`Contract '${contractName}'`, async () => {
    ...
    async function deployPausableExtMock(): Promise<{ pausableExtMock: Contract }> {
      ...
      let pausableExtMockFactory = await ethers.getContractFactory(`${contractName}Mock`);
