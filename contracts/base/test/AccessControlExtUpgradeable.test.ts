...
for (const contractName of ["AccessControlExtUpgradeable", "AccessControlEnumerableExtUpgradeable"]) {
  describe(`Contract '${contractName}'`, async () => {
    ...
    async function deployAccessControlExtMock(): Promise<{ accessControlExtMock: Contract }> {
      ...
      let accessControlExtMockFactory = await ethers.getContractFactory(`${contractName}Mock`);
