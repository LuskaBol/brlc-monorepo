...
for (const contractName of ["RescuableUpgradeable", "RescuableEnumerableUpgradeable"]) {
  describe(`Contract '${contractName}'`, () => {
    ...
    async function deployRescuableMock(): Promise<{ rescuableMock: Contract }> {
      ...
      let rescuableMockFactory = await ethers.getContractFactory(`${contractName}Mock`);
