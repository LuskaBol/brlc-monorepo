...
describe("Contract 'AccessControlExtUpgradeable'", async () => {
  ...
  async function deployAccessControlExtMock(): Promise<{
    accessControlExtMock: Contracts.AccessControlExtUpgradeableMock;
  }> {
    ...
    const accessControlExtMockDeployment = await upgrades.deployProxy(accessControlExtMockFactory);
    await accessControlExtMockDeployment.waitForDeployment();
    const accessControlExtMock = accessControlExtMockDeployment.connect(deployer);
    return { accessControlExtMock };
  }

  async function deployAndConfigureAccessControlExtMock(): Promise<{
    accessControlExtMock: Contracts.AccessControlExtUpgradeableMock;
  }> {
  ...
  describe("Function 'grantRoleBatch()'", async () => {
    ...
    describe("Is reverted if", async () => {
      it("The caller does not have the expected admin role", async () => {
        ...
        const accessControlExtMockViaAttacker = accessControlExtMock.connect(attacker);
  ...
  describe("Function 'setRoleAdmin()'", async () => {
    ...
    it("Is reverted if the caller does not have the owner role", async () => {
      ...
      await expect(accessControlExtMock.connect(attacker).setRoleAdmin(OWNER_ROLE, someRole))
        .to.be.revertedWithCustomError(accessControlExtMock, ERROR_NAME_ACCESS_CONTROL_UNAUTHORIZED_ACCOUNT)
