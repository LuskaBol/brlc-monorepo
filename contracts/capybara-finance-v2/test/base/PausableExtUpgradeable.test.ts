...
describe("Contract 'PausableExtUpgradeable'", async () => {
  ...
  async function deployPausableExtMock(): Promise<{ pausableExtMock: Contracts.PausableExtUpgradeableMock }> {
    // The contract factory with the explicitly specified deployer account
    let pausableExtMockFactory =
      await ethers.getContractFactory("PausableExtUpgradeableMock") as Contracts.PausableExtUpgradeableMock__factory;
    ...
    const pausableExtMockDeployment = await upgrades.deployProxy(pausableExtMockFactory);
    await pausableExtMockDeployment.waitForDeployment();
    const pausableExtMock = pausableExtMockDeployment.connect(deployer);

    return { pausableExtMock };
  }

  async function deployAndConfigurePausableExtMock(): Promise<{
    pausableExtMock: Contracts.PausableExtUpgradeableMock;
  }> {
