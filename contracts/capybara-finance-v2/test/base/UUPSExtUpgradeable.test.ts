...
describe("Contract 'UUPSExtUpgradeable'", async () => {
  ...
  async function deployContract(): Promise<{ uupsExtension: Contracts.UUPSExtUpgradeableMock }> {
    // The contract under test with the explicitly specified initial account
    const uupsExtensionDeployment = await upgrades.deployProxy(uupsExtensionFactory, []);
    await uupsExtensionDeployment.waitForDeployment();
    const uupsExtension = uupsExtensionDeployment.connect(deployer); // Explicitly specifying the initial account
